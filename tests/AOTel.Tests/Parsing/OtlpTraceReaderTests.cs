...
public class OtlpTraceReaderTests
{
    ...
    public void Enumerator_ExtractsSpanCorrectly_FromRawPayload()
    {
        ...
        var reader = new OtlpTraceReader(new ReadOnlySequence<byte>(payload));
    ...
    [Fact]
    public void Enumerator_ExtractsSpanCorrectly_AcrossSegmentBoundaries()
    {
        // The split between segments MUST occur directly in the middle of a VarInt length delimiter,
        // and another split MUST occur in the middle of the 16-byte TraceId.
        byte[] part1 = [
            0x0A, 0x88, 0x01, // resource_spans (Field 1, Length 136)
            0x12, 0x85, 0x01, // scope_spans (Field 2, Length 133)
            0x12, 0x82,       // spans tag 0x12, then first byte of length 130 (0x82) -- SPLIT HERE
        ];

        byte[] part2 = [
            0x01, // second byte of length 130 (0x01)

            // --- Span Data ---
            0x0A, 0x10, // trace_id (Field 1, Length 16)
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, // first 8 bytes of TraceId -- SPLIT HERE
        ];

        byte[] part3 = [
            0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, // last 8 bytes of TraceId
            ...
            0x7A, 0x52, // Field 15 (WireType 2, Length 82) to pad out to 130 bytes span length
        ];

        byte[] part4 = new byte[82];
        byte[] fullPart3 = new byte[part3.Length + part4.Length];
        Buffer.BlockCopy(part3, 0, fullPart3, 0, part3.Length);
        Buffer.BlockCopy(part4, 0, fullPart3, part3.Length, part4.Length);

        var sequence = CreateSegmentedSequence(part1, part2, fullPart3);
